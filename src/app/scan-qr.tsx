...
import { RiskPreviewSheet } from '@/components/RiskPreviewSheet';
...
export default function ScanQRScreen() {
  ...
  const [pendingSigningRequest, setPendingSigningRequest] = useState<string | null>(null);

  const isSigningRequest = (data: string) => {
    return data.startsWith('midori://sign') || data.startsWith('midori://dapp/sign');
  };
  ...
  const handleBarCodeScanned = useCallback(
    ({ type, data }: { type: string; data: string }) => {
      ...
      if (isSigningRequest(data)) {
        setPendingSigningRequest(data);
        return;
      }
  ...
  const handleCancelSigningRequest = useCallback(() => {
    setPendingSigningRequest(null);
    setScanned(false);
  }, []);

  const handleConfirmSigningRequest = useCallback(() => {
    if (!pendingSigningRequest) return;

    router.replace({
      pathname: '/authorize',
      params: { signingRequest: encodeURIComponent(pendingSigningRequest) },
    });
  }, [pendingSigningRequest, router]);
  ...
  return (
    <View style={[styles.container, { paddingTop: insets.top }]}>
      ...
      <RiskPreviewSheet
        visible={pendingSigningRequest !== null}
        title="Vista previa de riesgo"
        description="Este QR solicita una firma. Revisa el origen, la red y el desglose de fees antes de continuar."
        origin="QR / deep-link Midori"
        network="Segun solicitud"
        fees={{ networkFee: 'Estimado al confirmar', providerFee: '0 o segun proveedor', midoriFee: '0 para firma normal' }}
        onCancel={handleCancelSigningRequest}
        onConfirm={handleConfirmSigningRequest}
      />
    </View>
  );
}
